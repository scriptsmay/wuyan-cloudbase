exports.main = async (_event, _context) => {
  ...
  try {
    ...
    if (week) {
      ...
      res = await db.collection('weekly_story').orderBy('created_at', 'desc').limit(1).get();
    }
    ...
    try {
      const overviewRes = await db.collection('season_summaries').orderBy('updated_at', 'desc').limit(1).get();
      if (overviewRes && overviewRes.data && overviewRes.data.length > 0) {
    ...
    try {
      ...
      const liveRes = await db.collection('live_streams').where({ year, month }).get();
...
function normalizeStats(stats) {
  ...
  let winRateDiff =
    stats.win_rate_diff != null
      ? stats.win_rate_diff
      : stats.winRateDiff != null
        ? stats.winRateDiff
        : stats.win_rate && stats.win_rate.diff != null
          ? Math.round(stats.win_rate.diff * 1000) / 10
          : 0;
  ...
  const kdaDiff =
    stats.kda_diff != null
      ...
      : stats.kdaDiff != null
        ? stats.kdaDiff
        : stats.kda_ratio && stats.kda_ratio.diff != null
          ? stats.kda_ratio.diff
          : 0;
  const battlesDiff =
    stats.battles_diff != null
      ? stats.battles_diff
      : stats.battlesDiff != null
        ? stats.battlesDiff
        : stats.battles && stats.battles.diff != null
          ? stats.battles.diff
          : 0;
