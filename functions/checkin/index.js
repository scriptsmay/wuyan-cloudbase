...
const {
  ENV_ID, parseBody, getQuery, getHeader, resolveIdentity, successResponse,
  errorResponse, optionsResponse, getRequestId, getClientIp, shanghaiDate,
  normalizeClientId, isValidClientId, hashValue
} = require('./runtime')
...
exports.main = async (event) => {
  ...
  if (method === 'GET' && path.endsWith('/stats')) {
    try { return successResponse(await getStats(), requestId, origin) } catch (error) {
  ...
  try {
    ...
    if (method === 'POST' && /\/api\/checkins\/?$/u.test(path)) {
      ...
      if (!isValidClientId(clientId)) return errorResponse(400, 'INVALID_ARGUMENT', 'client_id 不合法', requestId, origin)
      if (Object.prototype.hasOwnProperty.call(body, 'subject_id') || Object.prototype.hasOwnProperty.call(body, 'date')) {
        console.warn('[checkin] ignored client-owned fields', { requestId, hasSubjectId: 'subject_id' in body, hasDate: 'date' in body })
      }
...
async function getStats() {
  ...
  return { date, today_count: Number(doc && doc.count || 0), updated_at: doc && doc.updated_at || new Date().toISOString() }
}
...
async function withTransactionRetry(work) {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    try { return await db.runTransaction(work) } catch (error) {
      lastError = error
