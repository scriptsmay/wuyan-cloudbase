...
exports.main = async (_event, _context) => {
  ...
  try {
    ...
    if (overview.data && overview.data.hero_stats) {
      ...
      console.log(
        `[sync] hero_top computed from hero_stats: ${heroTop.length} heroes (was ${(overview.hero_top || []).length})`
      );
    ...
    const summaryDoc = {
      season,
      season_name: overview.data && overview.data.career_summary ? overview.data.career_summary.last_season_id : season,
