...
async function fetchKplScheduleList(seasonId, timeout = 15000) {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('timeout', () => {
      req.destroy();
      reject(new Error('KPL API timeout'));
    });
...
function findMatchIndex(matches, update) {
  ...
  if (update.start_ts && update.team_a && update.team_b) {
    const idx = matches.findIndex(
      (m) =>
        !m.schedule_id && m.start_ts === update.start_ts && m.team_a === update.team_a && m.team_b === update.team_b
    );
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      const result = await db.runTransaction(async (transaction) => {
        const existing = await transaction.collection('match_schedules').where({ season_id: seasonId }).get();
        ...
        await transaction.collection('match_schedules').doc(doc._id).update(updateData);
